...
def process_image(image):
    ...
    undistorted = undistortimage(binary,
                                 ...
                                 params['newcameramtx'], 
                                 params['distCoeffs'])
    #undistorted = cv2.circle(undistorted, 
    #                (int(params['camera_center'][0]), 
    #                int(params['camera_center'][1])), 
    #                2, 255, 2)
    ...
    unwarped = cv2.warpPerspective(out_warped, Minv, (image.shape[1], image.shape[0]), flags=(cv2.INTER_LINEAR)) # | cv2.WARP_FILL_OUTLIERS))
...
if __name__ == '__main__':
    ...
    #    ret = cv2.imwrite('./undist2/main{:02d}.jpg'.format(i), dst)        
...
for i in trange(len(filenames)):
    ...
    undistorted = undistortimage(binary,
                             ...
                             params['distCoeffs'])
    warped, Minv = warp_perspective(undistorted, params['camera_center'])
    #ret = cv2.imwrite('./video_warped/main{:04d}.jpg'.format(i), warped)
    func_left, func_right = findlane(warped)
    ploty = np.linspace(0, warped.shape[0]-1, warped.shape[0]/2 )                     
    ploty = ploty.reshape(ploty.shape[0], 1)
    left_fitx = func_left(ploty)
    ...
    ret = cv2.polylines(out_warped,[p_left.astype(np.int32)], False, (0,0,255), 10)
    ret = cv2.polylines(out_warped, [p_righ.astype(np.int32)], False, (255,0,0), 10) 
    ret = cv2.imwrite('./video_warped/main{:04d}.jpg'.format(i), out_warped)
""" 
